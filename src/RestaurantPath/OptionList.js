...
const OptionList = props => {
    ...
    const [state, setState] = useState({
        optionViewState: false,
    })
    const tabledataset = {
        tableHead: ['#', 'รายการท็อปปิ้ง', 'ราคา (฿)', 'แก้ไข'],
        tableData: [
            ['1', 'ไข่ดาว', '5', 'แก้ไข'],
            ['2', 'ไข่เจียว', '5', 'แก้ไข']
        ]
    ...
    const addItem = () => {
        ...
        if (label != "" && value != "") {
            ...
            axios
                .post(`${baseURL}restaurant/options`, vara, config)
                .then((res) => {
                    if (res.status == 200 || res.status == 201) {
                        ...
                        setTimeout(() => {
                            props.navigation.navigate("OptionList");
                        }, 500)
                    }
                })
                .catch((error) => {
    ...
    return (
        <View style={styles.Tablecontainer}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 20, marginBottom: 30 }}>
                ...
                <TouchableOpacity onPress={() => props.navigation.navigate('OptionList', { restId: restId })}><Text style={styles.pageButton}>ท็อปปิ้ง</Text></TouchableOpacity>
            </View>
                    ...
                    renderItem={({ item }) =>
                        <>
                            <View style={[{ width: '100%', flexDirection: 'row' }]}>
                                <Text style={[{ flex: .2, padding: 8, fontFamily: 'pr-light', fontSize: 16, textAlign: 'center' }]}>{(item._id).substr(21, 24)}</Text>
            ...
            <Modal transparent={true} visible={state.optionViewState}>
                <View style={styles.ModelBackground}>
                    <View style={styles.ModalContainer}>
                        <View>
                            <Text style={styles.titleText}>ชื่อท็อปปิ้ง</Text>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 20 }}>
                            <TouchableOpacity style={styles.touchContainer} onPress={() => { addIten(), setState({ ...state, optionViewState: false }) }}><Text style={styles.submitModalBtn}>เพิ่ม</Text></TouchableOpacity>
                            <TouchableOpacity style={styles.touchBackContainer} onPress={() => { setState({ ...state, optionViewState: false }) }}><Text style={styles.cancelModalBtn}>ยกเลิก</Text></TouchableOpacity></View>
                    </View>
                </View>
            </Modal>
        </View>
